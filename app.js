/* =================================================================
   ...
   ================================================================
   M2 AMIS modules discovered from CELCAT data · Client-side .ics generation
...
const I18N = {
  fr: {
    ...
    heroSub: "Tous les événements et modules M2 AMIS, du 21 septembre 2026 au 31 octobre 2027.",
    ...
    pickerSub: "Tous les modules publiés par CELCAT sont sélectionnés automatiquement.",
  ...
  en: {
    ...
    heroSub: "All M2 AMIS events and modules from September 21, 2026 through October 31, 2027.",
    ...
    pickerSub: "All modules currently published by CELCAT are selected automatically.",
...
async function loadData() {
  ...
  try {
    ...
    availableModules = extractAvailableModules(celcatData);
...
// -----------------------------------------------------------------
// Render module cards discovered from CELCAT
// -----------------------------------------------------------------
function renderModules() {
  ...
  $$(".module-cb").forEach((cb) => {
    if (cb.checked) prev.add(cb.dataset.code);
  });
  const selectAllInitially = !$$('.module-cb').length;
  ...
  availableModules.forEach((mod) => {
    ...
    if (selectAllInitially || prev.has(mod.code)) {
      cb.checked = true;
      cb.dispatchEvent(new Event("change"));
    }
  });
...
function extractAvailableModules(rawEvents) {
  const modules = new Map();
  for (const raw of rawEvents) {
    for (const label of (raw.modules || [])) {
      const clean = cleanHtml(label);
      MODULE_CODE_RE.lastIndex = 0;
      const match = MODULE_CODE_RE.exec(clean);
      if (!match) continue;
      const code = match[1].toUpperCase();
      const name = clean.replace(new RegExp(`^${code}\\s*[-–]\\s*`, "i"), "") || code;
      modules.set(code, name);
    }
  }
  return [...modules].map(([code, name]) => ({ code, name })).sort((a, b) => a.code.localeCompare(b.code));
}
 ...
 */
function filterEvents(rawEvents, modules, includeExams, includeAll) {
  ...
  for (const raw of rawEvents) {
    ...
    const codes = extractModuleCodes(allText + " " + (raw.modules || []).join(" "));
    const matchingCodes = includeAll ? codes : codes.filter((c) => enrolledCodes.has(c));

    if (!includeAll && !matchingCodes.length && !isExam) continue;

    const category = eventCategory(raw);

    // Group filtering for TD/TP (exams always pass)
    if (!includeAll && !isExam && (category.startsWith("TD") || category.startsWith("TP"))) {
      ...
      if (groupNums.length) {
        ...
        for (const code of matchingCodes) {
          ...
          if (expected != null && groupNums.includes(String(expected).toUpperCase())) { match = true; break; }
        }
        if (!match) continue;
      }
    }
    ...
    } else if (includeAll || isExam) {
      summary = titleRaw ? cleanHtml(titleRaw) : category;
    } else {
      continue;
    }
...
function collectSelected() {
  ...
  $$(".module-cb:checked").forEach((cb) => {
    ...
    const mod = availableModules.find((m) => m.code === code);
    selected[code] = { name: mod ? mod.name : code };
  });
...
function generate() {
  ...
  try {
    const includeAll = Object.keys(modules).length === availableModules.length;
    const filtered = filterEvents(celcatData, modules, true, includeAll);
    if (!filtered.length) {
...
function subscribe() {
  ...
  const config = {
    ...
    includeAllEvents: includeAll,
    modules: Object.entries(modules).map(([code, mod]) => ({ code, name: mod.name })),
  };
...
function parseIcsModules(icsText) {
  ...
  for (const line of lines) {
    ...
    while ((m = MODULE_CODE_RE.exec(line)) !== null) {
      ...
      if (!modules[code]) {
        const mod = availableModules.find((x) => x.code === code);
        modules[code] = { name: mod ? mod.name : code };
      }
